...
`read_image()` returns the `distances` field alone, reshaped to `([n_envs,] height, width)`; misses carry `no_hit_value`. The point cloud is still available through `read().points`, unless the camera is created with `return_points=False`. Because a depth camera shares the raycasting backend with any lidar in the scene, the two cast against the same geometry.
