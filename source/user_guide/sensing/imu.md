...
The family is `*_noise` (white-noise standard deviation), `*_bias` (constant offset), `*_random_walk` (drift standard deviation), `*_cross_axis_coupling` (axis misalignment), and `*_resolution` (quantization step), plus the shared `delay` and `jitter` timing. Their exact meanings and defaults are in {doc}`the IMU reference </api_reference/engine/sensors/imu>` and the {doc}`sensors overview <index>`, and we recommend taking the values from your target hardware's datasheet rather than guessing them.
