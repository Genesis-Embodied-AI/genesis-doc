...
Set `CUDA_VISIBLE_DEVICES` and `QD_VISIBLE_DEVICE` together to the same index so simulation and any PyTorch tensors land on one device. Each process sees only that one GPU, so it always refers to it as index `0` internally.
