...
A {py:class}`DroneEntity <genesis.engine.entities.drone_entity.DroneEntity>` is a quadrotor whose actuation is its four propeller speeds. Set each propeller's angular velocity in **RPM** (revolutions per minute), and Genesis World converts those speeds into the aerodynamic forces that lift and steer the drone.
...
The hover RPM is model-specific: it is the value at which `4 · KF · rpm²` equals the drone's weight.
