...
Soft-body dynamics need small timesteps and several substeps for numerical stability. Setting `dt` on each soft solver's options keeps the rest of the scene at the global timestep, which is what the example does:
