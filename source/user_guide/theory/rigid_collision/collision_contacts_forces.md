...
The number of candidate pairs the broad phase may emit is bounded by the `max_collision_pairs` option on {doc}`RigidOptions </api_reference/engine/solvers/rigid_options>`. Exceeding it at runtime halts the simulation, so raise it for scenes with dense contact.
