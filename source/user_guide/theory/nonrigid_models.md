...
For the complementary question of which *solver* to use and how to configure a scene around it, see {doc}`/user_guide/physics/beyond_rigid_bodies`. For actuated muscles specifically, see {doc}`/user_guide/physics/soft_robots`. For how forces cross material boundaries, see {doc}`/user_guide/theory/solvers_and_coupling`.
