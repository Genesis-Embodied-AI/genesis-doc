...
We default to `approximate_implicitfast` for the factorization it saves, and its approximation holds on the damping and constraint forces a typical robot scene generates. Choose `implicitfast` when a scene is stiff enough that the correction visibly changes the trajectory. On an undamped dof under `Euler`, the velocity update is exactly `qvel += dt * qacc`.
