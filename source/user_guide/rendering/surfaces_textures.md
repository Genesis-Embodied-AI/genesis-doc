...
In the ray tracer, a light is an ordinary entity carrying an `Emission` surface. An environment map is the same idea applied to the background: an emissive image wrapped around the scene lights everything in it.
...
{py:class}`gs.options.FoamOptions <genesis.options.misc.FoamOptions>` splits the effect into three populations and gives each its own dissipation rate: `spray_decay` for airborne droplets, `foam_decay` for foam riding the surface, and `bubble_decay` for bubbles carried below it, where a larger value clears that population faster. The remaining fields size and tint the particles: `radius_scale` as a fraction of the fluid particle radius, `k_foam` as the count generated per frame, and `color` as their RGBA tint.
