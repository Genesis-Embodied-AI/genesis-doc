...
:::{note}
**Roadmap:** we are unifying rasterization and path tracing under Nyx as a single, sensor-based rendering interface, so Nyx will gradually replace both the Luisa backend below and the default rasterizer until all camera-based rendering in Genesis World goes through it.
:::
