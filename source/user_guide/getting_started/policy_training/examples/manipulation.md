...
The cameras are added with `scene.add_sensor(...)`; see {doc}`Camera sensors </user_guide/getting_started/sensors/camera_sensors>` for the sensor API and {doc}`Parallel simulation </user_guide/getting_started/parallel_simulation>` for how batched environments run.
...
By default this runs 2048 parallel environments (`-B/--num_envs`) for 300 iterations. More environments give PPO more samples per update; the ceiling is your GPU memory. See {doc}`Efficient environment design </user_guide/getting_started/policy_training/best_practices/efficient_environment>` for how to push batch sizes higher.
...
- {doc}`Camera sensors </user_guide/getting_started/sensors/camera_sensors>` for the rendering API behind the stereo observations.
