...
Do not guess where the syncs are. Attach the PyTorch profiler around a few steps and look for gaps between kernel launches, which are the CPU waiting on the GPU. See {doc}`/user_guide/advanced_topics/profiling` for the profiling setup and how to read Quadrants kernel timings.
...
- {doc}`/user_guide/advanced_topics/profiling`: measure kernel time and launch latency.
