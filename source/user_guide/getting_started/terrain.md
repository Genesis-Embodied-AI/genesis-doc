...
Genesis World provides first-class support for **height-field terrains** via the `gs.morphs.Terrain` morph.  A terrain is a static rigid object represented internally by a height map (for fast collision queries) and a watertight triangle mesh (for visualisation & SDF generation).
...
If you already have terrain data (for example from DEM files) you can feed it directly to Genesis World.  The only two numbers you need are the horizontal and vertical scales.

```python
import numpy as np
import genesis as gs

# 1. initialise Genesis World
...
The code above is essentially the same as `examples/rigid/terrain_subterrain.py` shipped with Genesis World.  Feel free to open the example for a complete runnable script.
...
When a terrain is created, Genesis World generates the height map, the watertight mesh for collision detection and the simplified mesh for visuals. One can enable caching of the height map when a terrain is first created for a given set of options by passing `name="my_terrain"`, which would later be loaded from cache without regeneration. This is useful for reconstructing randomized terrains exactly.
