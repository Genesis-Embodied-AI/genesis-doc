...
Genesis 仿真框架围绕一个模块化和可扩展的配置系统构建。该系统允许用户通过结构化配置对象灵活地组合和控制仿真的不同方面——从低级物理求解器到高级渲染选项。

为了帮助您理解这些组件如何协同工作，我们从 Genesis 场景通常如何初始化的高级模板开始。此模板展示了如何编排仿真设置、求解器选项和实体级配置。
...
- [仿真与耦合](#仿真--耦合)：定义全局仿真参数和不同求解器如何交互。
- [求解器](#求解器)：为不同仿真方法（例如刚体、流体、布料）配置物理行为。
...
此配置定义了仿真的全局结构以及不同物理求解器如何耦合。这些选项控制仿真循环的"骨架"，例如时间步进、稳定性和求解器互操作性。

- `SimOptions`：设置全局仿真参数——时间步长、重力、阻尼和数值积分器。
...
此配置控制实时可视化（在调试和开发期间有用）和最终渲染输出（用于演示、分析或媒体）。它控制用户如何与仿真进行视觉交互和感知。
