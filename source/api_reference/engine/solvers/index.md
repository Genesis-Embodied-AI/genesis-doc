...
You rarely construct a solver directly. Add an entity with a material and the scene routes it to the matching solver; configure that solver through its options object. For how to combine solvers in one scene and choose between them, see {doc}`/user_guide/physics/beyond_rigid_bodies` and {doc}`/user_guide/theory/coupling/index`.
