...
Reports the nearest distance from each probe point to the mesh surfaces of a set of tracked rigid links, clamped to `probe_radius` when nothing is in range. The `nearest_points` attribute holds the matching nearest points. See {doc}`the surface distance guide </user_guide/sensing/surface_distance>`.
