...
Tactile sensors turn a rigid link's surface into a grid of sensing points, or **taxels**, and report how that surface interacts with nearby geometry. They range from a per-probe contact flag to dense per-taxel force, torque, and elastomer-displacement fields that imitate a tactile skin. For the two contact-estimation families, probe layout, and the force models behind each estimate, see {doc}`the contact and tactile sensors guide </user_guide/sensing/tactile>`.
