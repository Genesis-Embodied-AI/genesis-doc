...
Configure almost every component in Genesis World through an **Options** class, and the scene builds it into a working object (`RigidOptions` -> `RigidSolver`, `IMU` -> `IMUSensor`, and so on). Since the options and the object are two halves of one component, we document each **Options** with the object it configures.
