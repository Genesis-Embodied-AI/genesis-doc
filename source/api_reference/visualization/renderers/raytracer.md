...
Photorealistic output depends on entity {doc}`surfaces </api_reference/entity/surface/index>` (metal, glass, plastic, emission) and on scene {doc}`lighting </api_reference/visualization/lights>`. For setup and a worked example, see {doc}`/user_guide/rendering/index`.
...
- {doc}`/api_reference/entity/surface/index`: surface materials for ray tracing
