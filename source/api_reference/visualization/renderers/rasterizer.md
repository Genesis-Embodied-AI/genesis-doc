...
The options class takes no parameters. You configure rasterizer behavior such as shadows, lights, and per-environment isolation (`env_separate_rigid`) on `gs.options.VisOptions` rather than on the renderer. See {doc}`/user_guide/rendering/index` for adding cameras and reading back images.
