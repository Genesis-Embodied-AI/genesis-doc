...
- **RayTracer:** there are no light objects. A light is an entity with an `Emission` {doc}`surface </api_reference/engine/entity/surface/emission>`, plus the optional `SphereLight` area lights below.
